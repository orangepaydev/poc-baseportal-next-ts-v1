...
import {
  ...
  searchApprovedUserGroupPermissionTargetsPage,
} from '@/lib/user-group-permissions';

type UserGroupPermissionPageProps = {
...
function buildPageHref(searchQuery: string, page: number) {
  ...
  return query
    ? `/admin/user-group-permission?${query}`
    : '/admin/user-group-permission';
}

export default async function UserGroupPermissionPage({
  searchParams,
}: UserGroupPermissionPageProps) {
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-[1.05rem] font-semibold tracking-tight text-slate-950">
              User Group Permission search
            </h2>
          </div>
        </div>

        <form
          action="/admin/user-group-permission"
