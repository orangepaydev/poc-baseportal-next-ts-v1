...
type UserGroupPermissionDetailPageProps = {
  params: Promise<{
    groupId: string;
  }>;
...
export default async function UserGroupPermissionDetailPage({
  params,
  searchParams,
}: UserGroupPermissionDetailPageProps) {
  ...
  const pendingRequests = await listPendingUserGroupPermissionRequests(
    session.organizationId
  );
  ...
  const canManage = permissionCodes.includes('GROUP_PERMISSION_WRITE');
  const canApprove = permissionCodes.includes('GROUP_PERMISSION_APPROVE');
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold tracking-[0.22em] text-cyan-700 uppercase">
              User Group Permission Detail
            </p>
          </div>

          <div className="flex w-full flex-wrap items-center gap-3 md:w-auto">
            <Button variant="outline" className="rounded-2xl" asChild>
              <Link href="/admin/user-group-permission">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              Current live permission assignments for this user group.
            </p>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-slate-200 bg-white">
                {detail.permissions.length === 0 ? (
                  <tr>
                    ...
                    >
                      No permissions are assigned to this user group.
                    </td>
                  </tr>
                ) : null}
