...
import { updateSystemCodeRequestAction } from '../../actions';
import { SystemCodeValuesEditor } from '../system-code-values-editor';

type EditSystemCodePageProps = {
  params: Promise<{
    systemCodeId: string;
  }>;
...
export default async function EditSystemCodePage({
  params,
  searchParams,
}: EditSystemCodePageProps) {
  ...
  if (!permissionCodes.includes('SYSTEM_CODE_WRITE')) {
    notFound();
  }
      ...
      <section className="rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {pendingRequest ? (
          <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-800">
            Editing is unavailable because a pending request already exists for this System Code.
          </div>
        ) : (
          <form action={updateSystemCodeRequestAction} className="mt-6 grid gap-6">
            <input name="systemCodeId" type="hidden" value={systemCode.id} />
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              ...
              <label className="space-y-2 text-sm font-medium text-slate-700">
                ...
                <select
                  name="status"
                  defaultValue={systemCode.status}
            ...
            <label className="space-y-2 text-sm font-medium text-slate-700">
              ...
              <input
                name="description"
                type="text"
                defaultValue={systemCode.description}
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 font-normal text-slate-900 transition outline-none focus:border-cyan-500 focus:bg-white"
              />
            </label>

            <SystemCodeValuesEditor existingValues={systemCode.values} />
